...
import { databaseCourseRegistrationsRoute } from "../../../routes/academicRoutes";
...
export default function CourseRegistrationsReport({ filters }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(databaseCourseRegistrationsRoute, {
    ...
    <Stack gap="md">
      ...
      {result ? (
        <ReportTable
          columns={COLUMNS}
          rows={result.rows}
          filename={`course_registrations_${session}_${String(semesterType).replace(/\s+/g, "_")}_${batch}`}
          badges={[
...
CourseRegistrationsReport.propTypes = {
